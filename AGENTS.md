...
#### 陳述級溯源（claim-level，raw 有結構化定位資訊時必須）
- 若來源 raw 帶有結構化定位資訊（YouTube transcript 的 `[MM:SS]` 時間戳、PDF 頁碼、逐字稿段落編號等），正文中的事實性陳述（數字、日期、人名、因果結論、跨來源推論）必須 inline 標註對應位置，例如：
  > 延遲主要來自同步 I/O `[12:03]`
- 低風險描述性文字（過場句、章節標題）可保留段落層級的來源位置，不需逐句標註。
- **目的**：重現使用者截圖看到的 YouTube 內建 Gemini 摘要那種「每個推論可一鍵回溯原始位置」的效果，而不是只給整篇 raw 檔案級別的模糊來源。
- **架構原因（2026-08-18 圓桌會議結論）**：歸納（有損壓縮）與溯源（無損映射）若在同一次生成 Prompt 裡「盡量兼顧」，容易產生假精確引用（citation laundering）。因此交接時上游 skill（如 `wiki-ingest-youtube`）必須明確標記「這批 raw 有時間戳結構」，下游才會切換到陳述級溯源模式，而不是靠模型自己判斷要不要加。
