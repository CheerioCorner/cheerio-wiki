---
title: "BlackBeltTechnology/pi-agent-dashboard: Real-time web dashboard for pi coding-agent sessions. Multi-session view, live chat mirroring, integrated terminal, diff viewer, pi-flows execution, and mobile-first remote control via mDNS or zrok tunnel. Coexists with pi's TUI — doesn't replace it."
description: "Real-time web dashboard for pi coding-agent sessions. Multi-session view, live chat mirroring, integrated terminal, diff viewer, pi-flows execution, and mobile-first remote control via mDNS or zrok tunnel. Coexists with pi's TUI — doesn't replace it. - BlackBeltTechnology/pi-agent-dashboard"
source_url: "https://github.com/BlackBeltTechnology/pi-agent-dashboard"
...
tags:
  ...
  - "web-clip"
---
# BlackBeltTechnology/pi-agent-dashboard: Real-time web dashboard for pi coding-agent sessions. Multi-session view, live chat mirroring, integrated terminal, diff viewer, pi-flows execution, and mobile-first remote control via mDNS or zrok tunnel. Coexists with pi's TUI — doesn't replace it.

> Source: [BlackBeltTechnology/pi-agent-dashboard: Real-time web dashboard for pi coding-agent sessions. Multi-session view, live chat mirroring, integrated terminal, diff viewer, pi-flows execution, and mobile-first remote control via mDNS or zrok tunnel. Coexists with pi's TUI — doesn't replace it.](https://github.com/BlackBeltTechnology/pi-agent-dashboard)
> Clipped: 2026-08-02

## PI Dashboard

[![PI Dashboard — command an army of pi agents from one tab](https://github.com/BlackBeltTechnology/pi-agent-dashboard/raw/develop/docs/cover.png)](https://github.com/BlackBeltTechnology/pi-agent-dashboard)
...
🌐 **Website & demo:** [blackbelttechnology.github.io/pi-agent-dashboard](https://blackbelttechnology.github.io/pi-agent-dashboard) — animated tour, screenshots, and install guide. 📝 **Changelog:** [`CHANGELOG.md`](https://github.com/BlackBeltTechnology/pi-agent-dashboard/blob/develop/CHANGELOG.md)
...
| [![Sessions overview — folders, branches, OpenSpec changes, live token spend](https://github.com/BlackBeltTechnology/pi-agent-dashboard/raw/develop/docs/screenshots/readme-overview.png)](https://github.com/BlackBeltTechnology/pi-agent-dashboard/blob/develop/docs/screenshots/readme-overview.png)   <sub><b>Overview</b> — sessions grouped by folder, branch &amp; OpenSpec context, live cost</sub> | [![Active session — chat, attached OpenSpec change, ask_user prompt, token gauge](https://github.com/BlackBeltTechnology/pi-agent-dashboard/raw/develop/docs/screenshots/readme-session.png)](https://github.com/BlackBeltTechnology/pi-agent-dashboard/blob/develop/docs/screenshots/readme-session.png)   <sub><b>Session</b> — chat, OpenSpec apply, interactive <code>ask_user</code>, context gauge</sub> | [![Settings — ports, spawn strategy, tunnel, resolved tools table](https://github.com/BlackBeltTechnology/pi-agent-dashboard/raw/develop/docs/screenshots/readme-settings.png)](https://github.com/BlackBeltTechnology/pi-agent-dashboard/blob/develop/docs/screenshots/readme-settings.png)   <sub><b>Settings</b> — ports, spawn strategy, zrok tunnel, tool resolution</sub> |
| --- | --- | --- |
...
Open **[http://localhost:8000](http://localhost:8000/)** in any browser. All active pi sessions appear automatically. See [Prerequisites](#prerequisites) for Node.js / build-tool requirements.
...
C++ build tools are typically **not** required — `node-pty` ships a Windows x64 prebuild. Install Visual Studio Build Tools only if the prebuild fails to load. See [docs/installation-windows.md](https://github.com/BlackBeltTechnology/pi-agent-dashboard/blob/develop/docs/installation-windows.md) for more detail (offline / tarball / nvm-windows caveats).
...
Live-reconfigurable via Settings → Advanced → "Background polling (OpenSpec)" or `PUT /api/config` — no server restart needed. See [docs/architecture.md](https://github.com/BlackBeltTechnology/pi-agent-dashboard/blob/develop/docs/architecture.md) for the cost model.
...
For migration from `@blackbelt-technology/pi-model-proxy`, see [`docs/migration/from-pi-model-proxy.md`](https://github.com/BlackBeltTechnology/pi-agent-dashboard/blob/develop/docs/migration/from-pi-model-proxy.md).
...
The bundled `pi-dashboard` skill ships a `/dashboard:*` slash-command namespace for driving the dashboard from any pi session without hand-writing curl.

Two classes:

- **LLM-free** (read-only, zero token cost) — the command body runs as bash and renders directly in chat; the LLM is never invoked. A footer *"ℹ ran locally — LLM not invoked"* appears beneath the output. Examples:
	```
	...
	```
- **LLM-bound** (mutations / judgment) — the command expands into a user message the agent interprets, then calls the matching REST endpoint. Examples:
	```
...
LLM-free templates opt in via `executable: bash` frontmatter and get `PI_DASHBOARD_PORT` / `PI_DASHBOARD_BASE` injected. Full list: [`.pi/skills/pi-dashboard/references/slash-commands.md`](https://github.com/BlackBeltTechnology/pi-agent-dashboard/blob/develop/.pi/skills/pi-dashboard/references/slash-commands.md).
...
| `pi-flows` | `git@github.com:BlackBeltTechnology/pi-flows.git` | strongly suggested | Flow dashboard, role aliases (`@planning`, `@coding`, …), subagent / flow\_write / flow\_results / agent\_write / ask\_user / skill\_read / finish tools |
...
For the slot taxonomy, the manifest schema, and the plugin context API, see the skill's reference docs (or the runtime: [`@blackbelt-technology/dashboard-plugin-runtime`](https://www.npmjs.com/package/@blackbelt-technology/dashboard-plugin-runtime)). The reference fixture is [`packages/demo-plugin/`](https://github.com/BlackBeltTechnology/pi-agent-dashboard/blob/develop/packages/demo-plugin).
...
Monorepo with npm workspaces — top-level layout only. See [AGENTS.md](https://github.com/BlackBeltTechnology/pi-agent-dashboard/blob/develop/AGENTS.md) for the full file-by-file index.
...
See [`docs/release-process.md`](https://github.com/BlackBeltTechnology/pi-agent-dashboard/blob/develop/docs/release-process.md) for the full cut-a-release workflow.

### Continuous integration

Every push to `develop` and every pull request against `develop` triggers [`ci.yml`](https://github.com/BlackBeltTechnology/pi-agent-dashboard/blob/develop/.github/workflows/ci.yml):
...
The publish workflow ([`publish.yml`](https://github.com/BlackBeltTechnology/pi-agent-dashboard/blob/develop/.github/workflows/publish.yml)) triggers on `v*` tags:
...
| `macos-15-intel` | macOS x64 | `.dmg` (Intel; last GitHub-hosted x86\_64 image, EOL 2027-08) |
...
All artifacts are uploaded to a **draft GitHub Release**. Release notes are extracted automatically from the matching `## [<version>]` section of [`CHANGELOG.md`](https://github.com/BlackBeltTechnology/pi-agent-dashboard/blob/develop/CHANGELOG.md).

### On-demand Electron build (CI dispatch)

Build a one-off installer for a feature branch. No release, no publish, no tag.

Workflow: [`.github/workflows/ci-electron.yml`](https://github.com/BlackBeltTechnology/pi-agent-dashboard/blob/develop/.github/workflows/ci-electron.yml). Trigger from GitHub Actions tab → **CI Electron (on-demand)** → **Run workflow** button.
...
**Requirements** (already configured in [`publish.yml`](https://github.com/BlackBeltTechnology/pi-agent-dashboard/blob/develop/.github/workflows/publish.yml)):
