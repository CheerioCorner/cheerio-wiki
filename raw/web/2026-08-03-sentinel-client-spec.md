...
source_url: "https://redis.io/docs/latest/develop/reference/sentinel-clients/"
...
> Source: [Sentinel client spec](https://redis.io/docs/latest/develop/reference/sentinel-clients/)
> Clipped: 2026-08-03
...
For details about how Redis Sentinel works, please check the [Redis Documentation](https://redis.io/docs/latest/operate/oss_and_stack/management/sentinel/), as this document only contains information needed for Redis client developers, and it is expected that readers are familiar with the way Redis Sentinel works.
...
Once the client discovered the address of the master instance, it should attempt a connection with the master, and call the [`ROLE`](https://redis.io/docs/latest/commands/role/) command in order to verify the role of the instance is actually a master.

If the [`ROLE`](https://redis.io/docs/latest/commands/role/) commands is not available (it was introduced in Redis 2.8.12), a client may resort to the `INFO replication` command parsing the `role:` field of the output.
...
If the client will contact a Sentinel with yet not updated information, the verification of the Redis instance role via the [`ROLE`](https://redis.io/docs/latest/commands/role/) command will fail, allowing the client to detect that the contacted Sentinel provided stale information, and will try again.
...
Symmetrically the client should verify with the [`ROLE`](https://redis.io/docs/latest/commands/role/) command that the instance is actually a replica, in order to avoid scaling read queries with the master.
...
The [Sentinel documentation](https://redis.io/docs/latest/operate/oss_and_stack/management/sentinel/) shows how clients can connect to Sentinel instances using Pub/Sub in order to subscribe to changes in the Redis instances configurations.
