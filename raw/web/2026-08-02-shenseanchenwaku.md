---
title: "ShenSeanChen/waku-agent: Waku Waku! Waku agent is your personal AI agent, on your own laptop, in code you can read in an afternoon — harness + loop + memory + eval"
description: "Waku Waku! Waku agent is your personal AI agent, on your own laptop, in code you can read in an afternoon — harness + loop + memory + eval - ShenSeanChen/waku-agent"
...
> Source: [ShenSeanChen/waku-agent: Waku Waku! Waku agent is your personal AI agent, on your own laptop, in code you can read in an afternoon — harness + loop + memory + eval](https://github.com/ShenSeanChen/waku-agent)
> Clipped: 2026-08-02
...
Meet **Waku** — a local-first personal assistant that shows the four pillars behind every serious agent: **Harness · Loop · Memory · Eval/LLM-Ops**. No frameworks hiding the good parts. Built for [Sean's AI Stories](https://www.youtube.com/@SeanAIStories).
...
[![waku-agent architecture — the whiteboard](https://github.com/ShenSeanChen/waku-agent/raw/main/docs/architecture-whiteboard.png)](https://github.com/ShenSeanChen/waku-agent/blob/main/docs/architecture-whiteboard.png)

> The system-design whiteboard from the series. Every box maps to a file — see [the whiteboard maps to the code](#the-whiteboard-maps-to-the-code).
...
[YouTube](https://www.youtube.com/@SeanAIStories) · [X](https://x.com/ShenSeanChen) · [LinkedIn](https://linkedin.com/in/shen-sean-chen) · [Instagram](https://www.instagram.com/sean_ai_stories) · [TikTok](https://www.tiktok.com/@sean_ai_stories) · [Discord](https://discord.gg/tvECErKcFr) · [哔哩哔哩](https://space.bilibili.com/479332937) · [小红书](https://www.xiaohongshu.com/user/profile/5cf02cfb0000000005014371) · [抖音](https://www.douyin.com/user/MS4wLjABAAAAWCkd62_e8q4n-S34LIL04HsYN3m03l8MFdVYZToojP8)
...
**Use the model you already pay for.** Anthropic (default), OpenAI, Gemini, DeepSeek, MiniMax, Kimi, GLM, OpenRouter (one key, hundreds of hosted models), OpenCode Zen, or OpenCode Go — set `WAKU_PROVIDER=`, paste the key, done. One dialect in the loop; a [~60-line adapter](https://github.com/ShenSeanChen/waku-agent/blob/main/waku/loop/models.py) handles the rest.
...
Every whiteboard from the videos lives in [`docs/whiteboards/`](https://github.com/ShenSeanChen/waku-agent/blob/main/docs/whiteboards) as an **editable `.excalidraw` source** — download one, drop it on [excalidraw.com](https://excalidraw.com/), and remix it for your own team:

| Chart | What it explains |
| --- | --- |
| [`k3-architecture.excalidraw`](https://github.com/ShenSeanChen/waku-agent/blob/main/docs/whiteboards/k3-architecture.excalidraw) | Kimi K3: the 16-of-896 MoE, KDA + AttnRes attention, why agent loops get cheap |
| [`pi-architecture.excalidraw`](https://github.com/ShenSeanChen/waku-agent/blob/main/docs/whiteboards/pi-architecture.excalidraw) | pi (72K-star coding agent): 4-tool core, extensions, one EventStream |
| [`waku-architecture.excalidraw`](https://github.com/ShenSeanChen/waku-agent/blob/main/docs/whiteboards/waku-architecture.excalidraw) | Waku itself — harness, loop, memory pillars, LLM Ops (editable rebuild of [the whiteboard](https://github.com/ShenSeanChen/waku-agent/blob/main/docs/architecture-whiteboard.png)) |

New charts land here with every video. If they help you, [a star](https://github.com/ShenSeanChen/waku-agent) keeps them coming — and [sponsoring](https://github.com/sponsors/ShenSeanChen) gets new whiteboards early.

## The whiteboard maps to the code

This diagram renders straight from the README (it's [Mermaid](https://mermaid.js.org/) text, not an image — edit it in a PR):
...
> *Architecture of **waku-agent** — built on the series ([@ShenSeanChen](https://github.com/ShenSeanChen)). Code is MIT; **this diagram is licensed CC BY-NC-SA 4.0** — reuse it with credit to the channel, not for commercial resale.*

Every box is one module (full version with every file path: [docs/architecture.md](https://github.com/ShenSeanChen/waku-agent/blob/main/docs/architecture.md)):

| Diagram box | Module |
| --- | --- |
| Gateway Interface (CLI / voice / Telegram / web) | [`waku/gateway/`](https://github.com/ShenSeanChen/waku-agent/blob/main/waku/gateway) |
| Ephemeral Agent Run → Working Memory | [`waku/runtime/session.py`](https://github.com/ShenSeanChen/waku-agent/blob/main/waku/runtime/session.py) |
| The Loop (LLM ↔ tools, end-loop guardrails) | [`waku/loop/agent.py`](https://github.com/ShenSeanChen/waku-agent/blob/main/waku/loop/agent.py) |
| Agentic Tools (schedule / note / message) | [`waku/tools/`](https://github.com/ShenSeanChen/waku-agent/blob/main/waku/tools) |
| Procedural Memory (SKILL.md, "how to act") | [`waku/memory/procedural/`](https://github.com/ShenSeanChen/waku-agent/blob/main/waku/memory/procedural) + [`skills/`](https://github.com/ShenSeanChen/waku-agent/blob/main/skills) |
| Semantic Memory (durable facts, profile) | [`waku/memory/semantic/`](https://github.com/ShenSeanChen/waku-agent/blob/main/waku/memory/semantic) |
| Episodic Memory (dated events, past chats) | [`waku/memory/episodic/`](https://github.com/ShenSeanChen/waku-agent/blob/main/waku/memory/episodic) |
| "Should we even retrieve?" gate | [`waku/memory/retrieval_gate.py`](https://github.com/ShenSeanChen/waku-agent/blob/main/waku/memory/retrieval_gate.py) |
| Consolidate after N chats → summarizer | [`waku/memory/consolidation.py`](https://github.com/ShenSeanChen/waku-agent/blob/main/waku/memory/consolidation.py) |
| Trace (1 trace per run) | [`waku/ops/tracing.py`](https://github.com/ShenSeanChen/waku-agent/blob/main/waku/ops/tracing.py) |
| Eval: deterministic vs LLM-as-judge | [`evals/deterministic/`](https://github.com/ShenSeanChen/waku-agent/blob/main/evals/deterministic) vs [`evals/judge/`](https://github.com/ShenSeanChen/waku-agent/blob/main/evals/judge) |
| Gate → Release | [`waku/ops/release_gate.py`](https://github.com/ShenSeanChen/waku-agent/blob/main/waku/ops/release_gate.py) |
...
Yes, there's a real agent loop, and it's [~95 lines of plain Python](https://github.com/ShenSeanChen/waku-agent/blob/main/waku/loop/agent.py) — no LangGraph, no hidden control flow:
...
The agent loops across two tools: [`search_web`](https://github.com/ShenSeanChen/waku-agent/blob/main/waku/tools/search.py) reads the web, it reasons over the results, then calls [`create_event`](https://github.com/ShenSeanChen/waku-agent/blob/main/waku/tools/calendar.py) once per match — several iterations in a single turn. You'll see `iter 4`, `iter 5` … on the Loop tab and the LOOP box pulse for each cycle. `search_web` works keyless via DuckDuckGo but that endpoint rate-limits bots, so for a clean take set a free `TAVILY_API_KEY` (see [`.env.example`](https://github.com/ShenSeanChen/waku-agent/blob/main/.env.example)).
...
Deterministic tests are plain pytest in [`evals/deterministic/`](https://github.com/ShenSeanChen/waku-agent/blob/main/evals/deterministic); judged ones use DeepEval in [`evals/judge/`](https://github.com/ShenSeanChen/waku-agent/blob/main/evals/judge). Keeping them apart is the whole point — conflating "did it do the thing" (a unit test) with "was it any good" (a scored judgement) is the most common eval mistake.
...
**The bug workflow (this is the discipline you show on camera):** when you catch a bug by using the thing live, you fix it AND add a deterministic case so it can never come back. A real example from this repo: the agent didn't know the current *time* and asked for it before scheduling "in 30 minutes" → fixed in [`session.py`](https://github.com/ShenSeanChen/waku-agent/blob/main/waku/runtime/session.py), locked forever by [`test_working_memory.py`](https://github.com/ShenSeanChen/waku-agent/blob/main/evals/deterministic/test_working_memory.py). Run `make gate` → green → the eval history records the run.
...
**A beautiful voice.** Out of the box it uses macOS `say` — and Waku auto-picks the nicest voice you have, preferring a downloaded Premium/Enhanced one (System Settings ▸ Accessibility ▸ Spoken Content ▸ System Voice) over the robotic built-ins. For the real neural upgrade, install [Kokoro](https://github.com/hexgrad/kokoro) — a fully local, offline British-butler voice that's picked up automatically, no env var needed:
...
The local SQLite database and `calendar.ics` stay authoritative. To also write `create_event` results to Google Calendar, install the opt-in extra and configure [Application Default Credentials](https://cloud.google.com/docs/authentication/provide-credentials-adc):
...
- **manage\_memory** — correct or forget a fact when you say it's wrong.
- **update\_soul** — save a standing preference you give it (lives in `SOUL.md`).
- **create\_skill** — when you teach it a repeatable workflow, it offers to save it as a skill (written to `.waku/skills/`, live the same session).
...
**Contribute one — it's just a markdown file.** Copy [`skills/TEMPLATE.md`](https://github.com/ShenSeanChen/waku-agent/blob/main/skills/TEMPLATE.md), PR it into [`skills/community/`](https://github.com/ShenSeanChen/waku-agent/blob/main/skills/community). CI validates the frontmatter. See [CONTRIBUTING.md](https://github.com/ShenSeanChen/waku-agent/blob/main/CONTRIBUTING.md).
...
These live in [`waku/tools/experimental.py`](https://github.com/ShenSeanChen/waku-agent/blob/main/waku/tools/experimental.py), OFF by default — `WAKU_EXPERIMENTAL=1` registers them.

**Sub-Agents is now LIVE.** `delegate_task` hands a coding job to [pi](https://github.com/earendil-works/pi) — Mario Zechner's minimal open-source coding agent — through its headless print mode (`pi -p "task"`). Waku stays the orchestrator (memory, context, evals); pi is the specialist contractor (read/bash/edit/write). Try it:
...
| --- | --- | --- |
| SQLite FTS5 keyword memory | Supabase pgvector semantic search | `WAKU_SEMANTIC_STORE=supabase` + [sql/init\_supabase.sql](https://github.com/ShenSeanChen/waku-agent/blob/main/sql/init_supabase.sql) — the exact schema from [launch-rag](https://github.com/ShenSeanChen/launch-rag) / [launch-agentic-rag](https://github.com/ShenSeanChen/launch-agentic-rag) |
...
[launch-rag](https://github.com/ShenSeanChen/launch-rag) · [launch-agentic-rag](https://github.com/ShenSeanChen/launch-agentic-rag) · [launch-agent-skills](https://github.com/ShenSeanChen/launch-agent-skills) · [launch-mcp-demo](https://github.com/ShenSeanChen/launch-mcp-demo) · [launch-DeepResearch-Backend](https://github.com/ShenSeanChen/launch-DeepResearch-Backend)

## Community

Star the repo, join the [Discord](https://discord.gg/7Ntxzm3eJ), and grab a [good first issue](https://github.com/ShenSeanChen/waku-agent/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — that link is the live list, so it's always current. Gateways, memory backends and community skills are all shaped to be first PRs; the easiest needs no Python at all (see [contributing a skill](https://github.com/ShenSeanChen/waku-agent/blob/main/CONTRIBUTING.md)).
...
MIT — see [LICENSE](https://github.com/ShenSeanChen/waku-agent/blob/main/LICENSE). Built by [@ShenSeanChen](https://github.com/ShenSeanChen) ([YouTube](https://www.youtube.com/@SeanAIStories) · [X](https://x.com/ShenSeanChen)).