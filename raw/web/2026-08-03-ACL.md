...
source_url: "https://redis.io/docs/latest/operate/oss_and_stack/management/security/acl/"
...
> Source: [ACL](https://redis.io/docs/latest/operate/oss_and_stack/management/security/acl/)
> Clipped: 2026-08-03
...
The Redis [`AUTH`](https://redis.io/commands/auth) command was extended in Redis 6, so now it is possible to use it in the two-arguments form:
...
2. You want to improve operational safety, so that processes or humans accessing Redis are not allowed to damage the data or the configuration due to software errors or manual mistakes. For instance, there is no reason for a worker that fetches delayed jobs from Redis to be able to call the [`FLUSHALL`](https://redis.io/commands/flushall) command.
...
By default there is a single user defined, called *default*. We can use the [`ACL LIST`](https://redis.io/commands/acl-list) command in order to check the currently active ACLs and verify what the configuration of a freshly started, defaults-configured Redis instance is:
...
Also, in the special case of the default user, having the *nopass* rule means that new connections are automatically authenticated with the default user without any explicit [`AUTH`](https://redis.io/commands/auth) call needed.
...
- `off`: Disallow the user: it's no longer possible to authenticate with this user; however, previously authenticated connections will still work. Note that if the default user is flagged as *off*, new connections will start as not authenticated and will require the user to send [`AUTH`](https://redis.io/commands/auth) or [`HELLO`](https://redis.io/commands/hello) with the AUTH option in order to authenticate in some way, regardless of the default user configuration.
...
- `~<pattern>`: Add a pattern of keys that can be mentioned as part of commands. For instance `~*` allows all the keys. The pattern is a glob-style pattern like the one of [`KEYS`](https://redis.io/commands/keys). It is possible to specify multiple patterns.
...
- `&<pattern>`: (Available in Redis 6.2 and later) Add a glob style pattern of Pub/Sub channels that can be accessed by the user. It is possible to specify multiple channel patterns. Note that pattern matching is done only for channels mentioned by [`PUBLISH`](https://redis.io/commands/publish) and [`SUBSCRIBE`](https://redis.io/commands/subscribe), whereas [`PSUBSCRIBE`](https://redis.io/commands/psubscribe) requires a literal match between its channel patterns and those allowed for user.
...
1. Using the ACL command and its [`ACL SETUSER`](https://redis.io/commands/acl-setuser) subcommand.
2. Modifying the server configuration, where users can be defined, and restarting the server. With an *external ACL file*, just call [`ACL LOAD`](https://redis.io/commands/acl-load).

In this section we'll learn how to define users using the [`ACL`](https://redis.io/commands/acl) command. With such knowledge, it will be trivial to do the same things via the configuration files. Defining users in the configuration deserves its own section and will be discussed later separately.

To start, try the simplest [`ACL SETUSER`](https://redis.io/commands/acl-setuser) command call:
...
- In the off status, so [`AUTH`](https://redis.io/commands/auth) will not work for the user "alice".
- The user also has no passwords set.
- Cannot access any command. Note that the user is created by default without the ability to access any command, so the `-@all` in the output above could be omitted; however, [`ACL LIST`](https://redis.io/commands/acl-list) attempts to be explicit rather than implicit.
...
Such user is completely useless. Let's try to define the user so that it is active, has a password, and can access with only the [`GET`](https://redis.io/commands/get) command to key names starting with the string "cached:".
...
Things are working as expected. In order to inspect the configuration of the user alice (remember that user names are case sensitive), it is possible to use an alternative to [`ACL LIST`](https://redis.io/commands/acl-list) which is designed to be more suitable for computers to read, while [`ACL GETUSER`](https://redis.io/commands/acl-getuser) is more human readable.
...
The [`ACL GETUSER`](https://redis.io/commands/acl-getuser) returns a field-value array that describes the user in more parsable terms. The output includes the set of flags, a list of key patterns, passwords, and so forth. The output is probably more readable if we use RESP3, so that it is returned as a map reply:
...
Using another [`ACL SETUSER`](https://redis.io/commands/acl-setuser) command (from a different user, because alice cannot run the [`ACL`](https://redis.io/commands/acl) command), we can add multiple patterns to the user:
...
It is very important to understand what happens when [`ACL SETUSER`](https://redis.io/commands/acl-setuser) is called multiple times. What is critical to know is that every [`ACL SETUSER`](https://redis.io/commands/acl-setuser) call will NOT reset the user, but will just apply the ACL rules to the existing user. The user is reset only if it was not known before. In that case, a brand new user is created with zeroed-ACLs. The user cannot do anything, is disallowed, has no passwords, and so forth. This is the best default for safety.
...
Will result in myuser being able to call both [`GET`](https://redis.io/commands/get) and [`SET`](https://redis.io/commands/set):
...
- **admin** - Administrative commands. Normal applications will never need to use these. Includes [`REPLICAOF`](https://redis.io/commands/replicaof), [`CONFIG`](https://redis.io/commands/config), [`DEBUG`](https://redis.io/commands/debug), [`SAVE`](https://redis.io/commands/save), [`MONITOR`](https://redis.io/commands/monitor), [`ACL`](https://redis.io/commands/acl), [`SHUTDOWN`](https://redis.io/commands/shutdown), etc.
...
- **bloom** - Data type: all Bloom filter related commands. <sup><a href="#cmd-note-1">1</a></sup>
- **cms** - Data type: count-min sketch related commands. <sup><a href="#cmd-note-1">1</a></sup>
- **connection** - Commands affecting the connection or other connections. This includes [`AUTH`](https://redis.io/commands/auth), [`SELECT`](https://redis.io/commands/select), [`COMMAND`](https://redis.io/commands/command), [`CLIENT`](https://redis.io/commands/client), [`ECHO`](https://redis.io/commands/echo), [`PING`](https://redis.io/commands/ping), etc.
- **cuckoo** - Data type: all Cuckoo filter related commands. <sup><a href="#cmd-note-1">1</a></sup>
- **dangerous** - Potentially dangerous commands (each should be considered with care for various reasons). This includes [`FLUSHALL`](https://redis.io/commands/flushall), [`MIGRATE`](https://redis.io/commands/migrate), [`RESTORE`](https://redis.io/commands/restore), [`SORT`](https://redis.io/commands/sort), [`KEYS`](https://redis.io/commands/keys), [`CLIENT`](https://redis.io/commands/client), [`DEBUG`](https://redis.io/commands/debug), [`INFO`](https://redis.io/commands/info), [`CONFIG`](https://redis.io/commands/config), [`SAVE`](https://redis.io/commands/save), [`REPLICAOF`](https://redis.io/commands/replicaof), etc.
...
- **json** - Data type: all JSON related commands. <sup><a href="#cmd-note-1">1</a></sup>
- **keyspace** - Writing or reading from keys, databases, or their metadata in a type agnostic way. Includes [`DEL`](https://redis.io/commands/del), [`RESTORE`](https://redis.io/commands/restore), [`DUMP`](https://redis.io/commands/dump), [`RENAME`](https://redis.io/commands/rename), [`EXISTS`](https://redis.io/commands/exists), [`DBSIZE`](https://redis.io/commands/dbsize), [`KEYS`](https://redis.io/commands/keys), [`SCAN`](https://redis.io/commands/scan), [`EXPIRE`](https://redis.io/commands/expire), [`TTL`](https://redis.io/commands/ttl), [`FLUSHALL`](https://redis.io/commands/flushall), etc. Commands that may modify the keyspace, key, or metadata will also have the `write` category. Commands that only read the keyspace, key, or metadata will have the `read` category.
...
- **tdigest** - Data type: all t-digest related commands. <sup><a href="#cmd-note-1">1</a></sup>
- **timeseries** - Data type: all time series related commands. <sup><a href="#cmd-note-1">1</a></sup>
- **topk** - Data type: all top-k related commands. <sup><a href="#cmd-note-1">1</a></sup>
- **transaction** - [`WATCH`](https://redis.io/commands/watch) / [`MULTI`](https://redis.io/commands/multi) / [`EXEC`](https://redis.io/commands/exec) related commands.
...
NOTE: Redis can also show you a list of all categories and the exact commands each category includes using the Redis [`ACL CAT`](https://redis.io/commands/acl-cat) command. It can be used in two forms:
...
As you can see, so far there are 29 distinct categories. Now let's check what command is part of the *geo* category:
...
Sometimes the ability to exclude or include a command or a subcommand as a whole is not enough. Many deployments may not be happy providing the ability to execute a [`SELECT`](https://redis.io/commands/select) for any DB, but may still want to be able to run `SELECT 0`.
...
First, remove the [`SELECT`](https://redis.io/commands/select) command and then add the allowed first-arg. Note that **it is not possible to do the reverse** since first-args can be only added, not excluded. It is safer to specify all the first-args that are valid for some user since it is possible that new first-args may be added in the future.
...
- R (Read): User supplied data from the key is processed, copied or returned. Note that this does not include metadata such as size information (example [`STRLEN`](https://redis.io/commands/strlen)), type information (example [`TYPE`](https://redis.io/commands/type)) or information about whether a value exists within a collection (example [`SISMEMBER`](https://redis.io/commands/sismember)).
...
Redis internally stores passwords hashed with SHA256. If you set a password and check the output of [`ACL LIST`](https://redis.io/commands/acl-list) or [`ACL GETUSER`](https://redis.io/commands/acl-getuser), you'll see a long hex string that looks pseudo random. Here is an example, because in the previous examples, for the sake of brevity, the long hex string was trimmed:
...
Using SHA256 provides the ability to avoid storing the password in clear text while still allowing for a very fast [`AUTH`](https://redis.io/commands/auth) command, which is a very important feature of Redis and is coherent with what clients expect from Redis.
...
When you are just specifying a few users directly inside the `redis.conf` file, you can use [`CONFIG REWRITE`](https://redis.io/commands/config-rewrite) in order to store the new user configuration inside the file by rewriting it.

The external ACL file however is more powerful. You can do the following:

- Use [`ACL LOAD`](https://redis.io/commands/acl-load) if you modified the ACL file manually and you want Redis to reload the new configuration. Note that this command is able to load the file *only if all the users are correctly specified*. Otherwise, an error is reported to the user, and the old configuration will remain valid.
- Use [`ACL SAVE`](https://redis.io/commands/acl-save) to save the current ACL configuration to the ACL file.

Note that [`CONFIG REWRITE`](https://redis.io/commands/config-rewrite) does not also trigger [`ACL SAVE`](https://redis.io/commands/acl-save). When you use an ACL file, the configuration and the ACLs are handled separately.
...
Sentinel does not need to access any key in the database but does use Pub/Sub, so the ACL rule would be the following (note: [`AUTH`](https://redis.io/commands/auth) is not needed since it is always allowed):
