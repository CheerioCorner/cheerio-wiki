...
url: https://modelcontextprotocol.io/docs/2026-07-28/tools/inspector/cli
...
> Source: [CLI client](https://modelcontextprotocol.io/docs/2026-07-28/tools/inspector/cli)
> Clipped: 2026-08-22
...
**The config file is the only durable way to give a run its [roots](https://modelcontextprotocol.io/specification/draft/client/roots):** there is no roots flag, and `--method roots/set` applies only to that one short-lived connection. Roots configured for a server are advertised at connect, so a server that calls `roots/list` (as `@modelcontextprotocol/server-filesystem` does, to learn its allowed directories) gets them.
