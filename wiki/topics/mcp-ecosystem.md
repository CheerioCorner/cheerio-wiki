...
- [[wiki/entities/neon-mcp-server|Neon MCP Server — 現代 MCP Server 設計參考架構]] — Neon Postgres 官方 MCP server，展示 OpenAPI SDK + ergonomic methods + neon tools 三層架構。 🛠️
...
- [[wiki/concepts/progressive-tool-discovery|Progressive Tool Discovery — 漸進式工具發現]] — MCP Client 端實作的三階段工具發現模式（Search → Inspect → Execute），取代把所有 tool 一次性塞進 context window。 🛠️
...
- [[wiki/sources/2026-09-08-mcp-just-got-a-whole-lot-better|MCP Just Got a Whole Lot Better]] — Neon Postgres：MCP server 設計從一對一映射演進到 ergonomic workflow tools + progressive tool discovery。 🛠️
