...
> 本頁主要記錄檔案分工與關鍵類別;深度型別設計獨立到 [[late-conversion]]、迴圈結構獨立到 [[minimal-agent-loop]]、可觀測性獨立到 [[observability-layer]]。

## 來源
- [[2026-02-10-pi-agent-core-design]] — 主要來源(語言物理位元 + 行號層次的解析)
- 程式碼位置:`https://github.com/badlogic/pi-mono/blob/main/packages/agent/src/`
- 由 [[pi-mono]] 主頁面統合;本頁是它的下層 runtime 焦點。
...
→ 讓 continue() 變得優雅的關鍵設計:**構造錯誤訊息也會成為上下文**(`AssistantMessage, stopReason: "error"`)。詳見 [[late-conversion]]。

## 五項設計原則(由本類別衍生出)
- **極簡主義**(5 個檔 / 1,500 行 / 4 工具 / < 1000 token 系統提示)
- **可觀測性**(三層事件)
- **可干預性**(steering / follow-up 雙 queue)
- **最晚轉換**→ 獨立頁 [[late-conversion]]
- **自我進化**(透過 bash 自我呼叫)

## 不做的事(立場,精準條列)
無 MCP / 無 sub-agents / 無 plan mode / 無權限檢查 / 無 maxSteps。
→ 對應 [[yolo-by-default]] 與 [[pi-mono]] 的「刻意不做」。

## 相關頁面
- Entities:[[pi-mono]]、[[mario-zechner]]
- Concepts:[[late-conversion]]、[[minimal-agent-loop]]、[[observability-layer]]、[[steering-followup]]
- Source:[[2026-02-10-pi-agent-core-design]]
