...
> Shared Wiki 的可跨專案重用概念 collection。

- `type: concept` 的 canonical pages 放在本 collection。
- 主題分類透過 frontmatter `topics` 與 `wiki/topics/` navigation 表示。
- 尚未確認的方案與研究問題放在 `wiki/discussions/`，已確認的全域選擇放在 `wiki/decisions/`。
