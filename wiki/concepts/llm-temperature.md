...
影片定義 Temperature 同時控制兩個特性：隨機性（randomness）與創造力（creativity）`[00:53]`。

| 溫度值 | 輸出特性 | 適用場景 | 影片來源 |
|--------|---------|---------|---------|
| **T → 0** | 極度果斷、路徑一致 | 邏輯推理、程式碼生成、結構化 JSON 輸出 | `[01:39]` |
| **T = 0.1–0.3** | 穩定、低隨機性 | 工具呼叫（Tool Calling）、航班檢索、路由決策 | `[02:58]` |
| **T = 0.7** | 平衡 | 一般對話（Hugging Face 熱門模型預設） | `[02:18]` |
| **T = 0.9–1.0** | 高多樣性、創造力強 | 創意寫作、圖像提示詞生成、發想 | `[03:06]` |
| **T > 1.0** | 極端拉平（部分模型可達 1.5） | 實驗性探索（罕見） | `[01:25]` |

> 影片指出標準範圍 0 到 1，部分場景可達 1.5 `[01:25]`。社群基準約 0.7（Hugging Face 熱門模型預設）`[02:18]`。

### 補充：數學機制（非影片內容，通用背景知識）

> ⚠️ 以下為 LLM 領域的通用背景知識，影片本身未提及這些數學原理。

Temperature 參數 T 透過縮放 Logits（`z_i / T`）影響 Softmax 機率分佈：
- **T → 0**：趨近貪婪解碼（Greedy Decoding），機率分佈極度尖銳
- **T = 1**：原始機率分佈不變
- **T > 1**：機率分佈被拉平，高機率 token 與低機率 token 的差距縮小
