...
### Tau 是什麼
- Pi 的 **Python port**，由 Pi 團隊內部開發 `[0:00:14]`
- 架構與 Pi **完全相同**：session 管理、skills、extensions、system prompt 都一樣 `[0:00:14]`
- 跨平台：Mac / Linux / Windows `[0:01:06]`
- 安裝方式：一行 script `[0:01:06]`

### TUI 差異（Textual vs Pi 原生 TypeScript）
- Pi 的 TUI 是 TypeScript 從零打造；Tau 用 **Textual**（Python terminal UI framework）`[0:01:24]`
- 指令大致相同，但有些 Tau 獨有功能：
...
### Session 管理（與 Pi 相同）
- Session 以 **tree**（非 list）儲存，每個 message 有 `parent` 指向前列 `[0:08:27]`
- 支援 **fork**：從任意 message 分支出新對話 `[0:08:45]`
- 儲存格式：**JSONL**（每行一個 JSON 物件）`[0:10:22]`
- 路徑：`.tau/sessions/<working-directory>/`
- `/tree` 檢視與導覽整棵 session tree `[0:09:05]`
- `/resume` 列出並恢復過去 sessions `[0:07:12]`

### Session 匯出
- `/export` — 匯出為 HTML（互動式審閱）或 JSONL `[0:11:45]`
- 可讓另一個 agent 分析匯出的 session（用 session ID 即可）`[0:13:54]`
- 適合測試 skill / tool / MCP server 後做 feedback loop

### Skills 系統（與 Pi 相同）
- Skills = procedural instructions in markdown，可附帶 scripts / templates
- 載入位置：`~/.agents/skills/`、`~/.tau/skills/`、`~/.pi/skills/` + project root `[0:16:13]`
- 明確呼叫：`/skill:<name>`
- Agent 可自動判斷並呼叫 skill（description 在 system prompt 中）`[0:17:12]`
- `/skills` 命令可即時檢視、讀 skill 內容、加入 context

### Custom Prompts（Slash Commands）
- 與 skills 的差異：
  - Skills = procedural，agent 知道它的存在（in system prompt）`[0:19:45]`
  - Prompts = 純文字替換，agent 不知道有 prompts（前端 level 替換）`[0:20:02]`
- 範例：`/issue` → 替換為完整 GitHub issue 開立 prompt

### Extensions
- 與 Pi **完全相容**，使用相同 events `[0:20:49]`
- 需 port（語言不同）但 API 相同 `[0:20:49]`
- 影片中示範：Ryan 的 sub-agent extension `[0:20:26]`
- `/reload` 重新載入 skills / prompts / extensions / themes `[0:21:11]`

### Themes
- 可自訂 TUI 主題 `[0:21:33]`
- tau-light、high contrast、Catppuccin Mocha 等 `[0:21:33]`
- JSON 檔案格式，可請 Tau 自動產生 `[0:21:33]`

### Provider 支援
- Subscription：Codex、Anthropic、GitHub Copilot `[0:04:31]`
- API key：OpenAI、Anthropic、Gemini 等 `[0:04:48]`
- Hugging Face inference providers（單一路由）`[0:04:48]`
- Custom provider `[0:04:48]`
