...
- **不再閱讀 AI 生成的程式碼**：這是提升生產力的唯一方式 `[00:59]`
- **用極端約束包圍 agents**：unit tests、Gherkin tests、QA procedures、quality metrics、mutation testing、test coverage `[01:06]`
- **高信心來自測試閘道**：程式碼必須通過所有約束和測試才能上線 `[01:15]`

### 爭議背景

- Uncle Bob（Robert C. Martin）是《Clean Code》和《Clean Architecture》作者 `[00:00]`
- 推文獲得近 300 萬次瀏覽 `[00:30]`
- 爭議部分來自他的個人形象（支持者與反對者立場鮮明）`[01:25]`

### 程式碼關鍵性光譜（来自 Fio 的影片）

| 位置 | 應用類型 | 閱讀程式碼的必要性 |
|------|---------|------------------|
| 低風險 | 單用戶 web app | 不需要閱讀 `[02:18]` |
| 中風險 | SaaS 付費平台 | 可選擇性閱讀 `[02:37]` |
| 高風險 | 人命關天系統 | 必須閱讀所有程式碼 |

### 驗證方法（不需閱讀程式碼）

1. **Manual QA** — 打開應用測試功能 `[04:38]`
2. **Unit Tests** — 自動化單元測試 `[04:50]`
3. **Integration Tests** — 整合測試 `[04:50]`
4. **Architecture Tests** — 架構測試（防止 agent 破壞架構）`[05:03]`
5. **Skills** — 告訴 agent 如何驗證自己產生的程式碼 `[05:14]`
6. **Custom Debuggers** — 自訂偵錯工具 `[05:21]`
7. **Harnesses** — 確保程式碼在產生過程中正確 `[05:24]`
8. **Mutation Testing** — 變異測試 `[05:31]`
9. **Code Coverage** — 程式碼覆蓋率（衡量測試覆蓋範圍）`[05:37]`

### 核心觀點

- **「所有 AI 程式碼都是 slop」是錯的**：AI 是在人類程式碼上訓練的，所以人類程式碼也是 slop `[03:36]`
- **可以用 AI 產生驗證程式碼**：產生更多測試、skills、harnesses 來驗證應用程式碼 `[06:05]`
- **程式碼只是 artifact**：真正的價值是業務功能，程式碼只是 facilitate value creation `[07:00]`
- **工程師角色轉變**：從「寫程式碼」轉向「驗證與工程」`[06:32]`

### 未來展望

- 軟體工程師的未來依然光明（強調「工程師」部分）`[07:50]`
- 需要重新定義開發者的角色 `[07:34]`
- 將製作另一部影片深入討論「如果 agents 產生大部分程式碼，什麼仍然重要」`[07:34]`
