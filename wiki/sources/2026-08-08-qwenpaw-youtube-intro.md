...
QwenPaw 是 AgentScope 開發的本地 AI 助理，以 Agent OS 架構 + 安全沙箱為核心，GitHub 近三萬星 `[01:01]`。
...
現有 AI 助理三大痛點：
1. **資料隱私與雲端鎖定** — 對話/檔案上傳第三方，有隱私外洩風險 `[01:40]`
2. **擴充性不足** — 功能固定，難以客製化工作流程 `[01:53]`
3. **安全性疑慮** — 執行本地程式碼如同開後門，LLM 幻覺可能產生惡意指令 `[01:59]`

### 核心架構：Agent OS

三大支柱：
- **Resources** — 透明獨立的檔案工作區 `[02:50]`
- **Governance** — Tool Guard + File Guard 存取規則引擎 `[02:58]`
- **Sandbox** — OS 核心層級隔離（Linux Bubblewrap 等）`[03:12]`

### Scroll Context + ReMe

- 三層記憶體系統，完整保留對話歷史（不破壞性摘要）`[03:29]`
- ReMe 長期記憶檢索，Agent 永不遺忘 `[03:38]`

### 應用場景

- **開發者**：Coding Mode（三欄 Web IDE），讀/編輯/審查程式碼 `[03:51]`
- **營運人員**：排程任務，自動抓取數據、產生報告、推送到 DingTalk/Discord `[04:06]`
- **技術愛好者**：Skills + Plugin 生態，打造個人自動化工作流 `[04:18]`

### 社群反應

正面：易用性、多通道整合、強大文件處理、主動記憶系統 `[04:44]`
顾虑：外掛生態早期、阿里巴巴色彩可能導致 Qwen 生態鎖定 `[05:03]`

### 產業影響

- 將 AI Agent 安全性從可選功能拉高到核心架構層次 `[05:22]`
- 可能迫使其他 Agent 框架重新檢視安全模型 `[05:37]`
- 基於 AgentScope 2.0 + ACP，未來有演進為多代理協作系統的潛力 `[06:02]`
