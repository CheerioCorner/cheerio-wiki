...
- **大多數工程師閱讀太多程式碼，但生成不夠多**
- **程式碼現在很便宜**：應該生成更多 code 來驗證重要的 code
- **不是要你減少閱讀，而是要你生成更多**：「I'm not telling you to make your code cheaper. I'm telling you to make more cheap code.」
...
| 時代 | 閱讀 | 寫入 | 合併 |
|------|------|------|------|
| **以前** | 1000 行 | 200 行 | 100 行 |
| **現在** | 1000 行 | 2000+ 行 | 500 行 |
...
- **10,000 行 JavaScript 組織檔案**：以前不合理，現在 code 免費所以合理
- **用 10 個 agent 測試 API**：用便宜模型測試 API 是否可用
- **自訂 debugger 和 compiler hooks**：以前依賴內建工具，現在自己造
- **壓力測試**：以前要找朋友幫忙，現在讓 agent 做
...
- **Code is useful for things other than merging**：程式碼不只是用來合併的
- **重要程式碼只是你工作的一小部分**：如果你 90% 的時間都在處理 Tier D 程式碼，那你接觸的程式碼還不夠多
- **創造力是關鍵**：找不到用 cheap code 驗證重要 code 的方法 → 你還不夠有創造力
