...
1. **封裝格式**：Skill 本質上是把一段專業知識或工作流程包成「隨插即用」的模組——核心是一個 `SKILL.md` 文字檔，開頭用 YAML frontmatter 宣告名稱和觸發時機，底下用 Markdown 寫實際指令，需要的話還能搭配腳本、範例、參考文件等輔助檔案。各框架共通的設計是「漸進式揭露」：agent 平常只記得每個 Skill 的名稱和一句話描述，很省 token；真正被用到時才把完整指令讀進來；如果還有更詳細的附加資料，也是要用到才讀，不會一開始就塞爆對話。

2. **撰寫規範**：怎麼寫出一個「會被正確觸發、會被正確使用」的 Skill？關鍵在那句「描述」——這是 agent 決定要不要用這個 Skill 的唯一依據，寫得太籠統，agent 該用的時候反而不會用（業界稱為 under-trigger）。好的做法是把「什麼情境該用」講清楚、甚至寫得主動一點；內文建議解釋「為什麼要這樣做」而不是只丟死板規則，讓模型能舉一反三處理沒預想到的邊界情況；篇幅上正文盡量精簡，太長的參考資料拆到附加檔案，需要時才讀。

3. **API 整合**：Skill 不是只能靠對話介面手動觸發，也可以程式化管理——例如透過 API/SDK 動態指定要載入哪些 Skill 目錄、暫時停用某些 Skill、或是替特定的 agent/session 指定它專屬能用哪些 Skill。這讓 Skill 系統能整合進更大的自動化流程或產品裡，不只是給人類互動用。

4. **企業治理**：當一個組織大規模導入 Skill（很多團隊都在寫、在用），就會冒出治理問題：這個 Skill 安不安全？誰能寫、誰能審、誰能發布？版本怎麼管、過時或有問題的 Skill 怎麼汰換？這塊處理的是「Skill 從個人生產力工具，變成組織共用資產」之後要面對的營運與風險管理。

5. **跨框架比較**：目前不同的 AI agent 框架（Claude Code、GitHub Copilot、Pi Agent 等）都各自實作了「Skill」這個概念，格式大致相容（都是 `SKILL.md`），但在觸發機制、載入時機、能不能組合成更複雜的多 agent 協作上，設計取捨很不一樣。這個專題的核心任務之一，就是把這些差異攤開來看，找出共通的本質限制，以及各框架各自的因應方式（詳細機制見本頁後續段落）。
