...
- [ ] W-2026-08-074 個人 AI 助理架構願景：最小規格 + 垂直切片 🆕 #ai-agent #architecture
  - next: **9/2 已完成**：三個架構缺口補完、兩處 schema 欄位拍板寫入、軟體架構圓桌共識定案、Excalidraw 第三欄「軟體架構（實作視角）」已畫（v0.8）。**下一步直接進入實作**——照圓桌建議的六步：① `pnpm init` + workspace + 四層 packages + tsconfig project references ② `packages/core`：四份 schema 從手寫 YAML 轉成 TypeScript Zod 定義 ③ `packages/events`：Drizzle + SQLite + initial migration ④ `packages/orchestration`：XState v5 狀態機 + Guard（ActorContext 驗證）+ 指數退避重試 ⑤ `packages/experts`：ExpertHarness + ChildProcessRunner + 零號專家 ⑥ 用 vitest 跑通四條測試路徑（正常／逾時重試／毒藥訊息／越權防護）
  ...
  - **2026-08-30 Cheerio 全貌與 MVP（Cheer × Claude 五輪對談收斂）**
    ...
    - **已拍板的架構決策**：
      ...
      18. **記憶四層機制定案（9/2）**：Episodic＝案件事件流封存 ＋ 每案結案摘要（SQLite ＋ FTS5，**不用向量**，理由是查詢形狀是「哪個案子、什麼時候」的精確查詢）／Semantic＝MVP 用純文字 MEMORY.md，長大後才換 Mem0 式向量／Procedural＝**不新建系統**，是一級紀錄的投影／Artifact＝檔案系統為真相 ＋ 索引表，**不切塊向量化**（產出物多為結構完整的 markdown，走 chunkless RAG 路線）。三條記憶紅線：核心不做相似度檢索（驗收不能因檢索漏撈而判錯）、只 retire 不 delete、記憶是參考資料不是最高指令。設計文件：[[work/designs/w074-memory-architecture|W-074 記憶架構設計]]
      19. **一級紀錄本身就是 decision-ledger（9/2 關鍵推論）**：decision-ledger 的 `intent → rationale → action → outcome` 跟我們的意圖紀錄 → 任務契約 → 事件流 → 驗收 verdict 逐欄對得上。Sakana AI 實測 decision-ledger 勝過 vector RAG 且更省 token，**所以效果最好的那一階記憶機制在 Cheerio 是治理需求的免費副產品**，不是額外成本。這條同時解釋了「主 Cheerio 不背記憶」為何不會讓系統變笨
      20. **自我改進迴路五階（9/2）**：Trace（事件流，已有）→ Eval → Diagnose → Gate → Release。**我們的 eval 資料是免費的**——每跑一次閉環就產生一筆 `(意圖, 交付物, verdict, 理由)`，而人類推翻主 Cheerio 的判定（決策 #9 第三條）就是黃金標籤。兩個指標：**驗收準確率**（判官好不好）與**一次過率**（派工好不好），都能從一級紀錄直接算。**紅線：可改 prompt／SKILL／policy／專家註冊表，不可改四態狀態機與轉換權限、一級紀錄必填欄位、驗收基準來源**——讓 agent 能改自己的驗收標準等於沒有驗收（reward hacking）。設計文件：[[work/designs/w074-self-improvement-loop|W-074 自我改進迴路設計]]
      21. **ChildProcessRunner 從一開始就要能殺「整棵進程樹」（9/5 決定）**：不是只殺直接子進程。理由——Node 的 `process.kill()` 在 Windows 上只殺該 PID，孫進程會變孤兒；而 local model runtime（llama-server router mode）官方說法是每個模型各自一個子進程，進程樹會是 `harness → llama-server → 各 model 子進程` 三層。做法：用**絕對路徑** `C:\Windows\System32\taskkill.exe /PID <pid> /T /F`（某些 Node 24 build 的 PATH 查找不可靠，會以非同步 error event 失敗，Pi Agent 自己的 repo 有對應 issue），並掛 error handler。OS 級的正解是 Windows Job Object + `KILL_ON_JOB_CLOSE`，但 Node 需要 native addon，先不上。**這條併入 9/2 未解問題「Windows SIGKILL 殺不掉子系命令」，用測試路徑 B（逾時重試流）一起驗**。已實測：`taskkill /T /F` 對單層 llama-server 清得乾淨；⚠️ 三層情境尚未驗（當時還沒有 GGUF 模型）
      22. **專家註冊表預留 `model_runtime` 欄位（9/5 決定）**：值域 `external | local`，MVP 全部填 `external`，不影響任何現有邏輯。理由——local model runtime（llama-server 等）**不是「專家」而是基礎設施**，不能綁在 `ExpertHarness.invoke()` 的生命週期上（每次調用重載模型要付數秒成本），正確形狀是 singleton + reference counting + health check，生命週期綁 harness process。放置位置建議獨立一層（如 `packages/runtime`），**不可放進 `core`**（零外部相依是四層 package 最有價值的約束）。**不進 MVP**：會同時引入 runtime 生命週期、Windows 進程樹、模型載入延遲三個新變數，違反「先跑通閉環再擴」。做的時機是零號專家四條測試路徑跑通之後，當作第一次「換 runtime」練習，屆時也會是 `execution_constraint`（L5/L6）的第一個非空值。設計討論：[[wiki/discussions/local-model-runtime-in-w074-architecture|Local Model Runtime 在 W-074/W-080 的定位]]、工具知識：[[wiki/entities/llama-cpp|llama.cpp]]
    ...
    - **下一步**：~~草擬三份 schema~~ ✅ 已完成初版（8/30），存放於 `work/designs/schemas/`：① [[work/designs/schemas/intent-record|意圖紀錄]] ② [[work/designs/schemas/task-contract|任務契約]] ③ [[work/designs/schemas/state-transition-rules|四態轉換規則]] ④ [[work/designs/schemas/event-stream|事件流]]。**驗收欄位格式已拍板（8/30）**：採混合式——驗收標準本身用自由文字（讓 LLM 在步驟⑤做語意比對），但⑥的狀態轉換結果（通過/退回/上呈）是確定性的規則引擎判定。精神：理解交給 LLM，權限交給規則引擎
  - **軟體架構規格圓桌會議（9/2 14:16，Claude+Gemini+Copilot 三方，2 輪提早收斂、無未解分歧）**：紀要在 `C:/Cheerio/.pi/round-table/20260902-141650/synthesis.md`（⚠️ 注意在專案根目錄的 `.pi/`，不是 vault 裡的 `.pi/`，work/history 的 wikilink 目前指不到）
    - **① 模組邊界＝pnpm workspace monorepo 四層 package**：`core`（Zod schema／領域型別／錯誤常數，**零外部相依**）／`events`（better-sqlite3 WAL + Drizzle ORM + migration + 事件流寫入）／`orchestration`（XState 狀態機 + Guard + 重試排程 + Case Manager）／`experts`（ExpertHarness 抽象 + ChildProcessRunner + 零號專家）。**邊界劃在「契約」（Zod schema + TS interface）上，不劃在進程/服務上**；四份 schema 是最重要的架構資產
    - **② Runtime＝單一 Node.js process、本地跑**：不拆微服務、不考慮多機。Harness 本質是「被 host（Claude Code 或 Pi）呼叫的 orchestration library」。零號專家用 `child_process.fork()` 隔離（比 worker_threads 簡單、硬性記憶體隔離、跨平台好），但 `ExpertHarness` 保留 optional `isolation_level`（`none|process|thread`）供無痛升級。所有路徑走環境變數/設定檔，不寫死本地路徑
    - **③ 持久化＝SQLite + Drizzle ORM**：IntentRecord／TaskContract／EventStream 進 SQLite（事件流 append-only + WAL），**StateTransitionRules 不是資料而是程式邏輯**（XState machine definition）。用 drizzle-zod 讓 Zod schema 直接對應 DB schema（一份定義兩種用途）、第一版就用 Drizzle migration（不手寫 SQL）。EventStream MVP 不分表但預留 `partition_key`（YYYY-MM）
    - **④ 專家調用介面＝統一輸入輸出，不統一執行機制**：`ExpertHarness.invoke(contract) → ExpertResult`；**Tier 差異在 `invoke()` 內部實作，不在介面上**（Tier A 直接 SDK 呼叫／Tier B spawn 外部 CLI／Tier C child_process 隔離含逾時與 SIGKILL）。**可觀測性以 EventStream 為單一真相來源**——Tier 差異反映在「發出了幾種 event_type」，不在 ExpertResult 上疊床架屋（Tier C 只保證 `expert.started`/`expert.completed`）
    - **⑤ 跟既有生態的邊界＝不是全新系統，是把現有生態「正式化」**：主 Cheerio ＝ 現在「Claude 角色」協調職能的結構化實作；Tier B 專家 ＝ 直接包現有 `chat-with-*` skills（不重寫）；MCP 工具層直接複用（W-074 只需把 `execution_constraint` 轉成 tool 白名單）；**Pi 繼續管巡檢/排程/跨 session，但狀態機轉換絕不能被 Pi 的排程邏輯觸發**——物理隔離做法：`transitionState()` 不 export 給 Pi 能 import 的任何 package，只當 `case-manager.ts` 私有函式，且所有送進 XState 的事件都帶 `ActorContext`（`ORCHESTRATOR_PI｜HARNESS_INTERNAL｜HUMAN_OPERATOR`）由 Guard 強制驗證
    - **⑥ 零號專家驗收＝四條測試路徑**（vitest integration test）：A 正常流／B 逾時重試流（SIGKILL + 指數退避）／C 毒藥訊息流（不合法 schema → 致命錯誤不重試）／D **越權防護流（Pi 偽造 COMPLETED 事件 → Guard 攔截 + 產生 security 審計事件）**。驗收準則：拿著 EventStream 就能跟人解釋「每一步是誰決定的、根據什麼規則、留下什麼證據」，中間沒有任何一步靠口頭確認
    - **四個未解問題（不擋 MVP）**：Tier B skills 的逾時機制怎麼跟 `max_retries` 銜接／drizzle-zod codegen 工具鏈要實驗確認／**Windows 上 `SIGKILL` 可能殺不掉子系命令，需 `taskkill /F /T /PID` 輔助**／`ExpertResult.traceId` 在 Drizzle 層怎麼外鍵
  - **開工前置研究（8/31，已完成）**：正式寫垂直切片前，Cheer 想先盤點業界既有 Agent Harness/Framework 全貌（四大廠牌路線 OpenAI Codex／DeepSeek Harness／Vercel AI SDK／Microsoft AutoGen+Semantic Kernel，加上 Python／TypeScript 兩大生態圈主流框架），重點比較架構與可觀測性、多 Agent 協作能力，供評估對照（不是決定要採用哪個，W-074 本身仍傾向自建 harness）。分工：Claude 指揮＋跑 `deep-research-intake` 收斂規格，Pi 執行 `deep-research-execute`（Gemini Notebook / nlm CLI）
    - ✅ 已完成：`research-report.md` 產出，最終 177 筆來源（84 原始 + 93 補研究），7 個子問題全部確認有實質內容
    - **過程中發現並修好的 bug**：Claude 直接讀 `answers.json` 逐一比對後發現，第 1 節（Codex）與第 4 節（Microsoft）的答案其實是 `query_answers.js` 問題分派邏輯錯位（送錯問題文字給 nlm query），分別誤答成 Python／TypeScript 生態圈內容——這不是 Gemini 幻覺，`question` 標籤本身是對的，純粹是程式送錯查詢字串。Codex 額外查出 notebook 裡原本**零筆**來源談 OpenAI Codex CLI 本身（被合併查詢稀釋掉），Microsoft 則有 8 筆既有來源只是沒被問到。派 Pi 補救：Microsoft 純重新查詢、Codex 補跑一次窄範圍 deep 研究（新增 93 筆來源），改完 `answers.json` 後用 `generate_report.js` 重新產生報告，Claude 逐項核對（讀新內容確認主題對得上、citations 數量、sources-final.json 筆數）後確認正確，改動前後都有 `.bak-before-*` 備份
    - **Codex 覆核這條線本次失敗（環境問題，非本次任務阻塞）**：原本想比照使用者要求派 Codex CLI 監看品質，但這台機器上 `codex exec` 的 Windows sandbox 建立機制壞掉（`CreateProcessAsUserW` 存取被拒，連最基本讀檔指令都跑不動），繞過 sandbox 的嘗試又被 Claude Code 權限分類器擋下，改成把摘錄直接貼進 prompt 純文字推理也卡住掛死。最後品質把關全靠 Claude 自己直接讀檔案＋WebFetch 查證（確認 DeepSeek Harness 的 Cordis 架構、Vercel Eve 皆為真實產品，非捏造）。**這個 codex CLI sandbox 壞掉的問題值得另開一個小任務查——會影響到 round-table skill 以後叫 Codex 參與圓桌**
    - **待處理的小 bug（不影響本次報告，留給之後）**：`query_answers.js` 的問題分派邏輯（`scripts/query_answers.js` 約 39-55 行附近）有索引錯位風險，未來其他多子問題的研究 job 可能重演同樣的錯位，值得找時間修
    - refs: `raw/deep-research/rc-20260831-001/spec.json`、`raw/deep-research/rc-20260831-001/research-report.md`
  - **核心本體程式語言決策（9/2，Cheer × Claude 討論拍板）：TypeScript**（非 C#，Cheer 熟悉 C# 但要求「最適合 W-074 的語言」而非順手的語言）
    - **決策依據**：核心本體實際要解決五件事——① schema-as-code 驗證（intent-record/task-contract/event-stream 三份 schema 要從手寫 YAML 變成可執行驗證）② 有守衛條件的狀態機（決策 #9 四態轉換，誰能觸發哪個轉換是明文權限規則）③ subprocess 編排（啟動 Pi/Claude Code/Gemini/Codex 當黑盒子）④ 可觀測性/追蹤（事件流要對齊業界 OTel GenAI 語意規範）⑤ 官方 Agent/MCP SDK 支援（決策 #12 harness tier 可升級，未來核心本體可能要自己講 MCP protocol，不永遠靠殼子代理）
    - **TypeScript 命中前三項的具體理由**：**Zod**（schema 定義本身即 runtime validator，型別從 schema 推導，一份定義兩種用途）+ **XState**（業界最成熟的 guarded state machine 函式庫，直接對應「誰能觸發哪個轉換」的需求）+ **Anthropic 官方 Claude Agent SDK／MCP SDK 首發語言之一**（Python 同列為首發，C# 是社群維護、功能通常落後）
    - **三方老實比較（非生態習慣偏好，是技術需求貼合度）**：Python 是很接近的第二名（Pydantic 在 schema-as-code 這項一樣強，只是型別保障較弱的狀態機函式庫拉低整體適配度）；C# 在 subprocess 編排／OTel 支援上打平，但官方 Agent/MCP SDK 落後是具體弱項，不是主觀偏好
    - **不影響 W-080**：ADO Agent 維持 C#/.NET 10，理由是企業整合對齊（ADO 是 Microsoft 產品、公司主力工具 GitHub Copilot），跟 W-074 核心本體要解決的問題形狀不同，兩個決策各自成立、互不衝突
  - **三個架構缺口已補（9/2，Cheer 指示「把缺的都補起來先」）**：① wiki 原本只有 context-rot／context-decay 兩個病症頁、缺對策頁 → 新增 [[wiki/concepts/context-engineering|Context Engineering]]（Write／Read／Handoff／Maintain 四個對策族，並釐清 Prompt／Context／Memory／Harness Engineering 分工）② Episodic／Artifact 記憶「待定」→ 決策 #18 定案 ③ 決策 #14 只有前提沒有迴路 → 決策 #20 補上五階迴路
    - **五種 Engineering 的落點已釐清**：Prompt→核心（只有翻意圖／生契約／驗收比對三個 prompt，量比一般 agent 小一個數量級）／Context→核心 ＋ 專家／Harness→調用層與自建底座／Loop→**專家內部**／Graph→**核心**。關鍵一句：**核心是 Graph（確定性 guarded 狀態機），專家內部才是 Loop（自主迭代）**，跟「理解交給 LLM、權限交給規則引擎」是同一條線
    - **兩處 schema 欄位已拍板並寫入（9/2，Cheer 同意 Claude 建議）**：① `TaskContract` 新增 `rejected_alternatives`（`option` / `why_rejected` / `consequence?`）——派工本身就是一次跨 harness 交接，Tier C 黑盒專家（MVP 零號專家正是）拿不到任何脈絡，沒這欄下次會重踩同一個坑 ② `StateTransitionEvent` 把 `verdict` / `verdict_reason` / `verdict_confidence` / `human_override` / `overrides_span_id` 從 `metadata` 自由文字升成結構化欄位，並一併留 `override_kind`（`misjudged` vs `changed_mind`）——不升的話之後算「驗收準確率／一次過率」得用 LLM 讀 metadata，變成 LLM 評估 LLM。`event-stream.md` 的 `state.transition` 與 `review.result` payload 同步對齊（原 `match/reason/confidence` 欄位名廢止）。兩者理由都跟決策 #16 加 model 欄位相同：現在幾乎零成本，事後補要動歷史資料格式
