...
- [ ] W-2026-08-074 個人 AI 助理架構願景：最小規格 + 垂直切片 🆕 #ai-agent #architecture
  - next: **9/2 三個架構缺口已補完**（Context Engineering 對策頁、記憶四層定案、自我改進迴路）。待 Cheer 拍板兩處 schema 欄位（`TaskContract.rejected_alternatives`、`StateTransitionEvent` 的 verdict／human_override 結構化）→ 再把「Cheerio 軟體架構（實作視角）」畫成 excalidraw 第三欄 → 然後動手寫**零號專家 wrapper**（TypeScript，Tier C，粗暴包裝一個外部 harness，唯一任務是把第一個正式專家造出來），驗證「① 意圖紀錄 → ② 任務契約 → ③ 專家執行 → ④ 比對 → ⑤ 狀態轉換 → ⑥ 事件流」全流程走得通。完整收斂內容見下方「2026-08-30 Cheerio 全貌與 MVP」與「核心本體程式語言決策（9/2）」
