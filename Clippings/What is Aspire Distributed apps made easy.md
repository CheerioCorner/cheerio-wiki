---
title: "What is Aspire? Distributed apps made easy"
source: "https://aspire.dev/get-started/what-is-aspire/"
author:
  - "[[Aspire]]"
published: 2026-06-02
created: 2026-09-06
description: "Aspire is a multi-language local dev-time orchestration tool chain for building, running, debugging, and deploying distributed applications."
tags:
  - "clippings"
---
## What is Aspire?
...
- **Aspire is** multi-language: choose a C# or TypeScript AppHost, then connect workloads across many runtimes.
- **Aspire isn’t** a replacement for your application framework.
- **Aspire isn’t** a cloud provider or production runtime.
- **Aspire isn’t** only for.NET-only solutions. See [Languages & runtimes](https://aspire.dev/languages-and-runtimes/).

You describe your architecture in an [AppHost](https://aspire.dev/get-started/app-host/), run it locally with [`aspire run`](https://aspire.dev/reference/cli/commands/aspire-run/), inspect it in the [Aspire Dashboard](https://aspire.dev/dashboard/), and [deploy](https://aspire.dev/deployment/) the same application model to your target environment. Aspire supports **C#** and **TypeScript** as AppHost authoring languages today while helping you orchestrate workloads written in C#, JavaScript, Python, Go, Java, Rust, PowerShell, and more. AI coding tools build on that same model through the Aspire CLI, skill files, and MCP server.
...
| --- | --- | --- |
| **Starting services** | Open 5 terminals, run 5 different commands, hope they start in the right order | Run `aspire run` — everything starts automatically. See [AppHost overview](https://aspire.dev/get-started/app-host/). |
| **Connection strings** | Hardcode `localhost:5432` everywhere, break production deploys | Service discovery handles it — same code works locally and in production. See [Service discovery](https://aspire.dev/fundamentals/service-discovery/). |
| **”Works on my machine”** | Different team members have different ports, different configs | One AppHost definition, everyone runs the same setup. See [First app tutorial](https://aspire.dev/get-started/first-app/). |
| **Debugging** | Attach debugger to each service individually | Debug your entire stack with a single . |
| **Finding logs** | Check 5 different terminal windows | Aspire Dashboard shows all logs in one place. See [Dashboard overview](https://aspire.dev/dashboard/overview/). |
| **Adding a database** | Install locally, configure connection, hope versions match | `builder.AddPostgres("db")` — containerized, versioned, consistent. See [Integrations](https://aspire.dev/integrations/overview/). |
| **Working across languages** | Stitch together separate scripts and conventions for C#, Node.js, Python, and more | Model the same distributed app in a single AppHost, including multi-language stacks. See [Languages & runtimes](https://aspire.dev/languages-and-runtimes/). |
| **Using AI coding agents** | Agents only see fragments of your setup and guess at runtime state | `aspire agent init` gives agents structured access to your app model, logs, traces, and resources. See [Use AI coding agents](https://aspire.dev/get-started/ai-coding-agents/). |
...
Want to make that app model available to AI coding tools? See [Use AI coding agents](https://aspire.dev/get-started/ai-coding-agents/).
...
Discover how Aspire powers your applications through its [`AppHost`](https://aspire.dev/get-started/app-host/).
...
Learn more about [Pipelines and App Topology](https://aspire.dev/deployment/pipelines/).